...
import { getTeamById } from '../utils/db';
...
export const PlayerCard: React.FC<PlayerCardProps> = ({ 
  ...
  useEffect(() => {
    if (player.teamId) {
      getTeamById(player.teamId).then(t => {
        if (t) setTeam(t);
      });
    } else {
        setTeam(null);
      ...
      <div className={`relative w-full h-full transform-style-3d flip-transition ${isFlipped ? 'rotate-y-180' : ''} will-change-transform`}>
        ...
        >
          ...
          <div className="mt-auto relative z-20 pb-6 px-5">
            ...
            <div className="grid grid-cols-6 gap-2 border-t border-black/10 pt-3">
               ...
               <StatBox label="DEF" value={player.stats.defending} />
               <StatBox label="PHY" value={player.stats.physical} />
            </div>
          </div>
        ...
        >
           ...
           <div className="relative z-10 h-full flex flex-col p-6">
              {/* Header */}
              <div className={`flex justify-between items-end mb-6 border-b-2 border-current/10 pb-2 ${theme.text}`}>
                <div>
                   <div className="flex items-center gap-2">
                     <Activity size={18} className="opacity-70" />
                     <h3 className="font-display font-bold text-xl uppercase tracking-widest drop-shadow-sm leading-none">Scouting</h3>
                   </div>
                   <span className="text-[10px] opacity-60 uppercase font-bold tracking-wider block mt-1 ml-0.5">Performance Report</span>
                </div>
                <div className="text-right">
                    <div className="text-2xl font-display font-bold leading-none">{player.overallScore}</div>
                    <div className="text-[9px] font-bold uppercase opacity-60">{player.position}</div>
                </div>
              </div>

              {/* Main Stats Container */}
              <div className={`flex-grow rounded-xl ${theme.badgeBg} backdrop-blur-md border border-white/20 shadow-inner flex flex-col p-4`}>
                 
                 {/* Detailed Stats List - Compact */}
                 <div className="space-y-1.5 flex-grow">
                    <StatRowBack label="Acceleration" value={player.stats.acceleration} />
                    <StatRowBack label="Sprint Speed" value={player.stats.pace} />
                    <StatRowBack label="Agility" value={player.stats.agility} />
                    <StatRowBack label="Stamina" value={player.stats.stamina} />
                    <StatRowBack label="Strength" value={player.stats.physical} />
                    <StatRowBack label="Positioning" value={player.stats.defending} />
                    <StatRowBack label="Vision" value={player.stats.passing} />
                 </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                 ...
                 <div className={`p-2 rounded-lg ${theme.badgeBg} border border-white/10 text-center flex flex-col justify-center shadow-sm`}>
                    <span className={`text-[9px] font-bold uppercase opacity-60 ${theme.text}`}>Goals</span>
                    <span className={`text-xl font-display font-bold ${theme.text}`}>{player.goals || 0}</span>
                 </div>
                 
                 {/* Assists */}
                 <div className={`p-2 rounded-lg ${theme.badgeBg} border border-white/10 text-center flex flex-col justify-center shadow-sm`}>
                    <span className={`text-[9px] font-bold uppercase opacity-60 ${theme.text}`}>Assists</span>
                    <span className={`text-xl font-display font-bold ${theme.text}`}>{player.assists || 0}</span>
                 </div>

                 {/* Matches */}
                 <div className={`p-2 rounded-lg ${theme.badgeBg} border border-white/10 text-center flex flex-col justify-center shadow-sm`}>
                    <span className={`text-[9px] font-bold uppercase opacity-60 ${theme.text}`}>Matches</span>
                    <span className={`text-xl font-display font-bold ${theme.text}`}>{player.matchesPlayed || 0}</span>
                 </div>

                 {/* Physique */}
                 <div className={`p-2 rounded-lg ${theme.badgeBg} border border-white/10 flex flex-col justify-center items-center gap-1 shadow-sm`}>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-sm font-bold ${theme.text}`}>{player.height}</span>
                        <span className="text-[8px] opacity-60 font-bold uppercase">cm</span>
                    </div>
                    <div className="w-8 h-px bg-current opacity-20"></div>
                    <div className="flex items-baseline gap-1">
                        <span className={`text-sm font-bold ${theme.text}`}>{player.weight}</span>
                        <span className="text-[8px] opacity-60 font-bold uppercase">kg</span>
                    </div>
                 </div>
              </div>

              {/* Footer */}
              <div className="mt-4 pt-2 border-t border-current/10 flex justify-between items-center opacity-80">
                 <div className="flex items-center gap-2">
                    {team ? (
                        <>
                            {team.logoUrl && <img src={team.logoUrl} className="w-4 h-4 object-contain" crossOrigin="anonymous"/>}
                            <span className={`text-[9px] font-bold uppercase tracking-wider ${theme.text}`}>{team.shortName}</span>
                        </>
                    ) : (
                        <span className={`text-[9px] font-bold uppercase tracking-wider ${theme.text}`}>Free Agent</span>
                    )}
                 </div>
                 <div className={`text-[9px] font-bold uppercase tracking-[0.2em] opacity-60 ${theme.text}`}>ELKAWERA</div>
              </div>
           </div>
        </div>
