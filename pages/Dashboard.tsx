...
import { getAllPlayers, deletePlayer, getAllTeams, getPlayerById } from '../utils/db';
import { Player, Team } from '../types';
import { Link, useNavigate } from 'react-router-dom';
import { Edit2, Trash2, Activity, Download, Search, Filter, PlusCircle, Trophy, Sparkles, User, Clock } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const loadData = async () => {
    if (user?.role === 'admin') {
      ...
      if (user.playerCardId) {
        const player = await getPlayerById(user.playerCardId);
        if (player) {
          setMyPlayer(player);
        }
      }
  ...
  // Listen for storage changes (when admin creates card for player)
  useEffect(() => {
    ...
    // Also check periodically for updates (in case same browser)
